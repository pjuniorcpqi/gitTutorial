app.controller("TimeSheetAdminController", function($scope, $uibModal, $localStorage, $http) {
	...
	$scope.changeUser = function(){
		if($scope.userSelected.id == null){
			toastr["error"]("Invalid User. Please, enter a valid name.");
		}else{
			initScopes();
		}
	};
	...
	function setUser(){
		if($scope.userSelected == null){
			...
			$scope.userId = $localStorage.principal.userId;
		}else{
			$scope.userName = $scope.userSelected.email;
			$scope.userId = $scope.userSelected.id;
		}
	}
	...
	function getAbsenceReasons(){
		$http.get("absenceReasons/")
		.then(function (response) {
			$scope.absenceReasons = response.data;
		}, function(response){
			toastr["error"]("Something went wrong. Please, contact an administrator.");
		});
	}
	...
	$scope.submit = function(day){
		...
		if(validateTimeLog(day)){
			...
			$http.post('timelogsAdmin/saveByAdmin/', day)
			.then(function (response) {
	...
	$scope.lockWorklogsFromMonth = function(timeSheets){
		$scope.modalInstanceLock  = $uibModal.open({
			templateUrl: 'views/timesheet/timeSheetLock.html',
			controller: 'ModalLockTimeSheetAdminController',
			scope: $scope,
			windowClass: 'large-Modal-delete'
		});

		$scope.modalInstanceLock.result.then(function (isLoad) {
			if(isLoad){
				initScopes();
			}
		});
	...
	$scope.unlockWorklogsFromMonth = function(timeSheets){
		var configHead = {header : {'Content-Type' : 'charset=UTF-8'}};
		var url = 'timelogsAdmin/unlockedByAdmin/';
		for(var x in timeSheets){
			for (var y in timeSheets[x].workLogs) {
				timeSheets[x].workLogs[y].timeInserted = createNumberFromHourMinute(timeSheets[x].workLogs[y].timeInserted);
			};
		}

		$http.post(url, timeSheets, configHead)
		.then(function (response) {
			toastr["success"]("Unlocked Worklogs successfully.");
			callBackSave();
		}, function(response){
			toastr["error"]("Something went wrong. Please, contact an administrator.");
		});
	...
	$scope.alreadyLocked = function(timeSheets){
		var locked = false;
		for(var x in timeSheets){
			for (var y in timeSheets[x].workLogs) {
				if(timeSheets[x].workLogs[y].revenueLock){
					locked = true;
					break;
				}
			};
	...
	$scope.toggleAbsence = function(day, reasonId){
		day.userRequesting = $localStorage.principal.username; 
		day.userName = $scope.userName;
		day.userId = $localStorage.principal.userId;
		day.absenceReasonId = reasonId;
		...
		if (reasonId > 0) {
			url = 'timelogsAdmin/markAsAbsent/';
		} else {
			url = 'timelogsAdmin/unmarkAsAbsent/';
		}
	...
	$scope.openModalDelete = function (day) {
		...
		$scope.modalInstanceDelete  = $uibModal.open({
			...
			controller: 'ModalTimeSheetAdminController',
			scope: $scope,
			windowClass: 'large-Modal-delete'
		});
...
app.controller('ModalTimeSheetAdminController', ['$scope','$http','$timeout','$localStorage', function($scope, $http,$timeout,$localStorage) {
	...
	$scope.processDelete = function() {
		...
		if(timeSheet.timeLog.id == null){
			...
			var url = 'timelogsAdmin/removeByAdmin/';
...
app.controller('ModalLockTimeSheetAdminController', ['$scope','$http','$timeout','$localStorage', function($scope, $http,$timeout,$localStorage) {
	var configHead = {header : {'Content-Type' : 'charset=UTF-8'}};
	$scope.erroMsg = [];
	$scope.successMsg = false;

	$scope.processLock = function(timeSheets) {
		var url = 'timelogsAdmin/lockedByAdmin/';
	...
	function callbackDelete () {
		$scope.modalInstanceLock.close(true);
	};

	$scope.cancelLock = function() {
		$scope.modalInstanceLock.dismiss("cancel");
